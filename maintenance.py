...
def importdata():
    data=pd.read_csv("E:\\Datasets\\26_03_DM_DataSet_Maintenance_modified.csv", header=0)
    return data
def splitdataset(data):
    X=data.values[:,1:6]
    Y=data.values[:,6]
    X_train, X_test, Y_train, Y_test=train_test_split(X,Y, train_size=90, test_size=30, random_state=1)
    return X, Y, X_train, X_test, Y_train, Y_test
def train_using_infogain(X_train, X_test, Y_train,x):
    clf_gain = DecisionTreeClassifier(
            criterion = "entropy",min_samples_leaf=x, random_state = 100)
...
def visualize(data, clf_gain):    
  #  graph = graphviz.Source( tree.export_graphviz(clf_gain, out_file=None,
    ...
    graph = graphviz.Source( tree.export_graphviz(clf_gain, class_names = clf_gain.classes_, out_file=None,filled=True, feature_names=data.columns[1:6]))
...
def main():
    ...
    clf_gain_5 = train_using_infogain(X_train, X_test, Y_train,3)
